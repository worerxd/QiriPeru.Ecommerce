...
namespace QiriPeru.Ecommerce.API.Controllers
{
    public class UsuarioController : BaseApiController
    {
        ...
        [HttpGet("pagination")]
        public async Task<ActionResult<Paginaton<UsuarioDto>>> GetUsuarios([FromQuery] UsuarioSpecificationParams usuarioParams)
        {
            var spec = new UsuarioSpecification(usuarioParams);

            var usuarios = await _seguridadRepository.GetAllWithSpecAsync(spec);
            ...
            //if (totalUsuarios % usuarioParams.PageSize != 0)
            //{
            //    totalPages = (totalUsuarios / usuarioParams.PageSize) + 1;
            //}
            //else
            //{
            //    totalPages = totalUsuarios / usuarioParams.PageSize;
            //}

            var data = _mapper.Map<IReadOnlyList<Usuario>, IReadOnlyList<UsuarioDto>>(usuarios);

            return Ok(
                new Paginaton<UsuarioDto>
                {
                    Count = totalUsuarios,
                    Data = data,
